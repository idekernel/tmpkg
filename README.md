...
docker run --net=host -d -e CONFIGPROXY_AUTH_TOKEN=$TOKEN --name=tmpkg -v /var/run/docker.sock:/docker.sock idekernel/tmpkg python orchestrate.py --logging=debug --image='idekernel/kernel:notebook' --command='jupyter notebook --no-browser --port {port} --ip=0.0.0.0 --NotebookApp.base_url=/{base_path} --NotebookApp.port_retries=0 --NotebookApp.allow_origin="\*" --NotebookApp.token="" --NotebookApp.disable_check_xsrf=True'
